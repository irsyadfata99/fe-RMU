...
export function MemberDebtTable({
  debts,
  onView,
  onPay,
}: MemberDebtTableProps) {
  if (debts.length === 0) {
    return (
              ...
              <TableCell>
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    ID: {debt.member?.uniqueId}
                  </p>
                  {debt.member?.whatsapp && (
                    <p className="text-xs text-muted-foreground">
                      WA: {debt.member.whatsapp}
                    </p>
                  )}
                </div>
              </TableCell>
              <TableCell className="text-sm">
                {debt.member?.regionName || "-"}
              </TableCell>
              <TableCell className="text-sm">
                {debt.sale?.saleDate ? formatDate(debt.sale.saleDate) : "-"}
              </TableCell>
